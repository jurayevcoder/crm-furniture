...
import { useProductType } from "../../service/productType";
...
const update = (num) => {
  useProductType.typeList(num).then((res) => {
    console.log(res);
...
const addType = () => {
  ...
  useProductType
    .typeCreate(type)
    .then(() => {
      update(localStorage.getItem("category_page"));
    })
    .catch((err) => {
      console.log(err);
...
const paginate = (num) => {
  localStorage.setItem("category_page", num);
  update(num);
};
  ...
  useProductType
    ...
    .then((res) => {
      if (res.status === 200) {
        ...
        if (localStorage.getItem("category_page") >= pagination.totalPage) {
          update(pagination.totalPage);
        } else {
          update(localStorage.getItem("category_page"));
        }
      }
