...
import { productStore } from "../../stores/products/productStore.js";
...
const listUpdate = (num) => {
  useProduct.list(num).then((res) => {
    ...
    count.value = res?.data?.pagination?.totalCount;
    store.SET_LIST(res?.data?.records);
  });
};
...
const addProduct = () => {
  const product = {
    ...
    category_id: productInfo.type,
  };

  useProduct
    .create(product)
    .then(() => {
      listUpdate(localStorage.getItem("product_page"));
    })
    .catch((err) => {
      console.log(err);
    });
...
const paginate = (num) => {
  localStorage.setItem("product_page", num);
  listUpdate(num);
};
  ...
  useProduct
    ...
    .then((res) => {
      if (res.status === 200) {
        ...
        if (localStorage.getItem("product_page") >= pagination.totalPage) {
...
<template>
  <div class="p-3">
    <h2 class="uppercase dark:text-white text-gray-900 text-4xl text-center">
      Mahsulotlar
    </h2>
            ...
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
              ...
              <div>
                ...
                >
                  ...
                  <option v-for="el in state.LIST" :key="el.id" :value="el.id">
                    {{ el.name }}
                  </option>
                </select>
              </div>
