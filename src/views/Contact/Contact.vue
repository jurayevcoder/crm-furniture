...
import { useContacts } from "../../service/contact";
...
const listUpdate = (num) => {
  useContacts.list(num).then((res) => {
    ...
    state.SET_LIST(res?.data?.records);
  });
};
...
const addContact = () => {
  ...
  useContacts
    .create(contact)
    .then(() => {
      listUpdate(localStorage.getItem("contact_page"));
    })
    .catch((err) => {
      console.log(err);
    });
...
const paginate = (num) => {
  localStorage.setItem("contact_page", num);
  listUpdate(num);
};
  ...
  useContacts
    ...
    .then((res) => {
      if (res.status === 200) {
        ...
        if (localStorage.getItem("contact_page") >= pagination.totalPage) {
...
<template>
  <div class="p-3">
    <h2 class="uppercase dark:text-white text-gray-900 text-center text-4xl">
      Kontaktlar
    </h2>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-3">
                    ...
                    <span
                      v-else
                      class="bg-red-300 text-center p-2 rounded-xl block w-[60px]"
                      >Mavjud emas</span
                    >
                  </td>
                  <td class="px-4 py-3">
